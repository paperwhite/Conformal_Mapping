...
double Edge::kuv()
{
    ...
    Point p3 = m_halfedge[0]->he_next()->target()->point();
    Point p4 = m_halfedge[1]->he_next()->target()->point();
    if(p1==p4)
        cout<<"SAME"<<endl;
    double alpha = ((p0-p3)*(p1-p3))/(((p0-p3)^(p1-p3)).norm());
    double beta = ((p0-p4)*(p1-p4))/(((p0-p4)^(p1-p4)).norm());
    
    m_kuv= 0.5*(alpha+beta);
   // cout<<m_kuv<<endl;
    return m_kuv;
}
